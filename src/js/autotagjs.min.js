...
(b=U(b))&&y(b)},la=function(a,c){c%=2;var b=q(a),d=x(b),f=d.indexOf(a);return 0===f&&-1===c?b.previousSibling&&x(b.previousSibling).pop():f===d.length-1&&1===c?b.nextSibling&&x(b.nextSibling).shift():d[f+c]},ma=function(){var a=w().endContainer;m(a)&&S(a)&&(A(a,0),n(a));var c=a.textContent,b=a.parentNode.classList;b.contains("atg-pilot")&&(a.nodeValue=c.replace(/[ \u00a0]/g,""),b.remove("atg-pilot"),n(a));if(c=Z(a.textContent))a=A(a,c,!0).pop(),n(a);X(e.querySelectorAll("br"))},na=function(a,c){var b=
...
function(a){var c=a.which||a.keyCode||0;8!=c&&da();var b=w();if(b)if(8==c){if(b.collapsed){var d=b.startContainer,e=b.startOffset;c=q(d);if(!pa(b,k.CLEAR_LIST))if(r(d)||r(c)||V(d,e)){if(e=c.previousSibling,u(e)){e=z(e);d=z(c);if(r(d))n(e.lastChild);else{r(e)&&P(e);b=[];for(d=d.firstChild;d;)b.push(d),d=d.nextSibling;0<b.length&&W(b,e)}y(c)}}else{c=d;b=q(c);d=x(b);var g=d.indexOf(c);c.nodeValue=c.textContent.splice(e-1,1,"");r(b)?ca(z(b),!0):r(c)?(y(c.parentNode),c=d[g-1],n(c)):n(c,e-1)}}else qa(b);
...
window.clipboardData&&(b=window.clipboardData.getData("Text"));var g=1E4>b.length;b=b.split("\n");1<b.length&&na(c);c=d;for(var h=0;h<b.length;h++){var k=b[h];if(0<k.length){k=t(R(k));H(k.parentNode,c.parentNode);r(c)&&y(c.parentNode);var m=1,l=k.textContent,n=l.length;do{var p=Z(l.slice(0,m));p?(k=A(k,p,!0).pop(),l=k.textContent,n=l.length,m=1):m+=1}while(m<=n&&g)}null!==b[h+1]&&(e=F(e,{addPilotNode:!0}),c=x(e).shift())}K({startContainer:d,endContainer:c});a.preventDefault()});document.addEventListener("selectionchange",
