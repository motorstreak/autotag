...
window.clipboardData&&(b=window.clipboardData.getData("Text"));var g=1E4>b.length;b=b.split("\n");1<b.length&&na(c);c=d;for(var h=0;h<b.length;h++){var k=b[h];if(0<k.length){k=t(R(k));H(k.parentNode,c.parentNode);r(c)&&y(c.parentNode);var m=1,l=k.textContent,n=l.length;do{var p=Z(l.slice(0,m));null!==p?(k=A(k,p,!0).pop(),l=k.textContent,n=l.length,m=1):m+=1}while(m<=n&&g)}null!==b[h+1]&&(e=F(e,{addPilotNode:!0}),c=x(e).shift())}K({startContainer:d,endContainer:c});a.preventDefault()});document.addEventListener("selectionchange",
